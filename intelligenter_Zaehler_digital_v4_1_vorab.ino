...
v4.1_vorab Version: angepasste Muster für Verbrauchserkennung
...
unsigned long pcComInitTime = 15L * 1000L; //15sek. Initialisierungszeit für PC Verbindung
...
char payload[4];
char MusterLeistung[5] = {0, 0, 0, 0, 0}; //Vergleichswerte, die anzeigen, dass eine Leistungsangabe folgt
char MusterVerbrauch[4] = {001, 252, 144, 221};  //Vergleichswerte, die anzeigen, dass eine Verbrauchsangabe folgt
float WirkVerbrauch;
float WirkLeistung;
...
*/
//-----------------------------------------------------
char ssid[50] = "test123"; // Wlan SSID
char password[50] = "init"; // Wlan Passwort
...
void energyCounterData(){
  ...
  while (j < 200){
    ...
    (incomingByte[j + 3] == MusterVerbrauch[3])){   //&& (incomingByte[j + 4] == MusterVerbrauch[4])
